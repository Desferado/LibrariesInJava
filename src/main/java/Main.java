...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        for (int n = 0; n < employees.length; n++) {
            ...
            if (StringUtils.containsAny(n1, "0123456789-+!/,.@#$%^&*_~")
              ||StringUtils.isEmpty(n1)) {
                    throw new RuntimeException();
                }
            String firstNameEmployee = StringUtils.capitalize(StringUtils.lowerCase(n1));
            System.out.println("Введите Отчество сотрудника");
            Scanner name2 = new Scanner(System.in);
            String n2 = name2.nextLine();
            if (StringUtils.containsAny(n2, "0123456789-+!/,.@#$%^&*_~")||StringUtils.isEmpty(n2)) {
                throw new RuntimeException();
            }
            String middleNameEmployee = StringUtils.capitalize(StringUtils.lowerCase(n2));
            System.out.println("Введите Фамилию сотрудника");
            Scanner name3 = new Scanner(System.in);
            String n3 = name3.nextLine();
            if (StringUtils.containsAny(n3, "0123456789-+!/,.@#$%^&*_~")||StringUtils.isEmpty(n3)) {
                throw new RuntimeException();
            }
            String lastNameEmployee = StringUtils.capitalize(StringUtils.lowerCase(n3));
